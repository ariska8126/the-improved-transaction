...
public interface AttendanceRepository extends JpaRepository<Attendance, String> {
    ...
    //review 
    @Query(value = "SELECT att.* FROM approval a JOIN users u ON a.approval_requester_id = u.user_id "
            + "JOIN attendance att ON att.user_id = u.user_id JOIN request r ON a.request_id = r.request_id "
    ...
    //get user by id
    @Query(value = "SELECT * FROM users WHERE users.user_token = :id", nativeQuery = true)
    public Optional<Users> getUsersByToken(@Param("id") String id);

    @Query(value = "select role_name from users join role on role.role_id = users.role_id WHERE users.user_id = :id", nativeQuery = true)
    ...
    @Query(value = "SELECT IF(EXISTS(SELECT * FROM users WHERE users.user_token = :token),1,0)", nativeQuery = true)
    ...
    // For reports
    @Query(value = "select u.user_fullname, u.user_id, d.division_name, a.* from users u join attendance a on u.user_id = a.user_id join division d "
            + "on d.division_id = u.division_id where a.user_id = :id", nativeQuery = true)
    Iterable<Attendance> atteanceReport(@Param("id") String id);

    //select last 10 attendance
    @Query(value = "SELECT att.* FROM attendance att JOIN users u ON att.user_id = u.user_id JOIN bootcamp_detail bd ON u.user_id = bd.user_id "
            + "JOIN bootcamp b ON b.bootcamp_id = bd.bootcamp_id JOIN attendance_status ats on ats.attendance_status_id = att.attendance_status_id "
            + "WHERE b.bootcamp_id = :id AND ats.attendance_status_id = '1' AND att.attendance_date = curdate() "
            + "ORDER BY att.attendance_time DESC LIMIT 10", nativeQuery = true)
    public List<Attendance> findLastAttendanceByBootcampId(@Param("id") String id);
    //select last 10 attendance

    @Query(value = "SELECT att.* FROM attendance att JOIN users u ON att.user_id = u.user_id JOIN bootcamp_detail bd ON u.user_id = bd.user_id JOIN bootcamp b "
            + "ON b.bootcamp_id = bd.bootcamp_id JOIN attendance_status ats on ats.attendance_status_id = att.attendance_status_id WHERE b.bootcamp_id = :id "
            + "AND att.attendance_date = curdate() AND ats.attendance_status_id = '2' OR ats.attendance_status_id = '3' OR ats.attendance_status_id = '4' "
            + "OR ats.attendance_status_id = '7' ORDER BY att.attendance_time DESC LIMIT 10", nativeQuery = true)
    ...
    @Query(value = "SELECT b.* FROM bootcamp b JOIN bootcamp_detail bd ON b.bootcamp_id = bd.bootcamp_id WHERE user_id = :id", nativeQuery = true)
    ...
    @Query(value="select * from attendance where attendance_id = :id", nativeQuery = true)
